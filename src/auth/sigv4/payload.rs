...
//!
//! Three values land in the canonical request directly:
...
//!
//! Every streaming sentinel (`STREAMING-AWS4-HMAC-SHA256-PAYLOAD*` and the
//! ECDSA variants) is rejected up front with `UnsupportedSignature` because
//! PR 1 doesn't implement chunk-level verification (PR 2 of #63) — silently
//! allowing them through would either skip signature checks or produce
//! confusing `SignatureDoesNotMatch` errors.
...
impl PayloadHashForSigning {
    ...
    /// Whether the verifier still needs to read the request body to confirm
    /// it matches the signed digest.
...
pub fn classify_payload_header(
    ...
    // Any streaming sentinel we don't handle yet (HMAC chunked / ECDSA
    // chunked) is fail-closed in this PR.
    if value.starts_with("STREAMING-AWS4-HMAC-SHA256-PAYLOAD")
        || value.starts_with("STREAMING-AWS4-ECDSA-P256-SHA256-PAYLOAD")
    {
        return Err(S3Error::unsupported_signature(
            "signed aws-chunked streaming uploads are not supported in strict mode; \
             tracked in PR 2 of issue #63",
            request_id,
        ));
    }
...
mod tests {
    ...
    #[test]
    fn test_streaming_hmac_sentinel_unsupported() {
        let err = classify_payload_header("STREAMING-AWS4-HMAC-SHA256-PAYLOAD", rid())
            .expect_err("hmac streaming is fail-closed in PR 1");
